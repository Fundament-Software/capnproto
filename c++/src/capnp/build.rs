...
fn main() -> eyre::Result<()> {
    ...
    for capnp_header in CAPNP_HEADERS.into_iter().chain(CAPNP_COMPAT_HEADERS) {
        ...
        println!(
            ...
            capnp_header_file
                .to_str()
                .ok_or_else(|| eyre!("non–UTF-8 path: {:?}", capnp_header_file))?
        );
    ...
    for capnp_private_header in CAPNP_PRIVATE_HEADERS {
        ...
        println!(
            ...
            capnp_private_header_file
                .to_str()
                .ok_or_else(|| eyre!("non–UTF-8 path: {:?}", capnp_private_header_file))?
        );
        fs::copy(
            &*capnp_private_header_file,
    ...
    for capnp_extra in CAPNP_EXTRAS {
        ...
        println!(
            ...
            capnp_extra_file
                .to_str()
                .ok_or_else(|| eyre!("non–UTF-8 path: {:?}", capnp_extra_file))?
        );
