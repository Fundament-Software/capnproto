...
fn main() -> eyre::Result<()> {
    ...
    for capnpc_header in CAPNPC_HEADERS.into_iter() {
        ...
        println!(
            ...
            capnpc_header_file
                .to_str()
                .ok_or_else(|| eyre!("non–UTF-8 path: {:?}", capnpc_header_file))?
        );
        fs::copy(
            &*capnpc_header_file,
    ...
    for capnpc_private_header in CAPNPC_PRIVATE_HEADERS {
        ...
        println!(
            ...
            capnpc_private_header_file
                .to_str()
                .ok_or_else(|| eyre!("non–UTF-8 path: {:?}", capnpc_private_header_file))?
        );
        fs::copy(
            &*capnpc_private_header_file,
    ...
    for capnpc_source in CAPNPC_SOURCES {
        ...
        println!(
            ...
            capnpc_source_file
                .to_str()
                .ok_or_else(|| eyre!("non–UTF-8 path: {:?}", capnpc_source_file))?
        );
    ...
    for capnpc_extra in CAPNPC_EXTRAS {
        ...
        println!(
            ...
            capnpc_extra_file
                .to_str()
                .ok_or_else(|| eyre!("non–UTF-8 path: {:?}", capnpc_extra_file))?
        );
