...
impl Cfg {
    ...
    pub fn import_propagated_definitions(&mut self) -> eyre::Result<&mut Self> {
    ...
    pub fn export_propagated_definitions(&mut self, build: &mut Build) -> eyre::Result<&mut Self> {
        println!(
            "cargo:kj-build-propagated-definitions={}",
            serde_json::to_string(&self.propagated_definitions)
                .with_context(|| eyre!("propagated definitions serialization"))?
        );
...
pub trait BuildExt {
    fn propagate_definitions(&mut self, cfg: &mut Cfg) -> eyre::Result<&mut Self>;
}
